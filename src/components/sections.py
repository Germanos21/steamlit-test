    ...
    with st.container(border=True):
        ...
        with col1:
            try:
                ...
                st.markdown(f"<div class='price-metric'>\U0001F4B0 Price: AED {price:,.2f}</div>", unsafe_allow_html=True)
            except (ValueError, TypeError):
                st.markdown("<div class='price-metric'>\U0001F4B0 Price: N/A</div>", unsafe_allow_html=True)
        with col2:
...
def show_supplier_card(supplier: Dict[str, Any]) -> None:
    with st.container(border=True):
        ...
        with st.container():
            st.markdown(f"<div class='product-title'>{supplier['name']}</div>", unsafe_allow_html=True)
...
def show_ebay_search_form() -> None:
    ...
    try:
        ...
        # Move category button OUTSIDE the form
        st.markdown("""
            <style>
        ...
        with col_input:
            st.markdown("")
            # This input is inside the form below
